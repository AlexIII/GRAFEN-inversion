...
class Solver(object):
	def __init__(self, useOldx0 = False, gamma = 400):
		#self.ref_field - target field f [provided]
		#self.vRightDir - right-side vector = B^T(f), B - forward problem operator [provided]
		...
		#rDir, zDir - temporary computations [rDir provided as a copy of vRightDir (for point coorditates)]
		...
		#input data
		self.ref_field = "layered_field.grd"
		...
		self.gammaWeights = (
			Aop.mapGrids(self.path+self.x, (lambda gd: np.ones(gd.data.shape)*self.gamma ))
		...
		copyfile(self.ref_field, self.curFieldFile)
		self.copyReplace(self.rDir, self.vRightDir)
